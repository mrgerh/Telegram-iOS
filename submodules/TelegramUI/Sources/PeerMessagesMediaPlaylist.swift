...
final class SingleFileMediaPlaylistItem: SharedMediaPlaylistItem {
    ...
    init(file: TelegramMediaFile) {
        self.file = file
        self.id = SingleFilePlaylistItemId(fileId: file.id!)
    }
    ...
    var displayData: SharedMediaPlaybackDisplayData? {
        if file.isVoice {
            ...
            let performer = "file.musicAuthor"
            return SharedMediaPlaybackDisplayData.music(title: title, performer: performer, albumArt: nil, long: false, caption: nil)
        }
        return nil
    }
}
...
final class SingleFileMediaPlaylist: SharedMediaPlaylist {
    ...
    init(file: TelegramMediaFile) {
        self.file = file
        self.id = PeerMessagesMediaPlaylistId.singleFile(file.id!)
        self.updateState()
    }
